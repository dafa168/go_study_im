...
//添加好友 userid 用户id  dstid对方id
func (service *ContactService) AddFriend(userid, dstid int64) error {
	tmpUser := model.User{}
	b, e := DbEngine.Where("mobile = ? ", dstid).Get(&tmpUser)
	fmt.Println(b, e)
	//如果没有找到
	if tmpUser.Id == 0 {
	...
	//查询是否已经是好友
	// 条件的链式操作
	x, err := DbEngine.Where("ownerid = ?", userid).
		And("dstobj = ?", dstid).
...
func (service *ContactService) SearchCommunity(userId int64) ([]model.Community) {
	...
	err := DbEngine.Where("ownerid = ? and cate = ?", userId, model.CONCAT_CATE_COMUNITY).Find(&conconts)
	if err != nil {
		log.Fatal(err)
	}
	...
	err = DbEngine.In("id", comIds).Find(&coms)
	if err != nil {
		log.Fatal(err)
	}
	return coms
...
func (service *ContactService) SearchFriend(userId int64) ([]model.User) {
	conconts := make([]model.Contact, 0)
	objIds := make([]int64, 0)
	err := DbEngine.Where("ownerid = ? and cate = ?", userId, model.CONCAT_CATE_USER).Find(&conconts)
	if err != nil {
		log.Fatalln(err)
	}
	for _, v := range conconts {
	...
	err = DbEngine.In("id", objIds).Find(&coms)
	if err != nil {
		log.Fatalln(err)
	}
	return coms
